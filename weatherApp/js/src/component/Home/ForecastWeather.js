...
const Forecast = props => {
    ...
    const func = (arr, size) =>{
        const results = arr.reduce((acc, curr, i) => {
            if ( !(i % size)  ) { 
                //short array groupby days
                const shortArray = arr.slice(i, i + size)
                //average datas
                var avg = Array.from(shortArray.reduce(
                ...
                new Map())).reduce( 
                    (acc, [name, values]) => 
                        Object.assign(acc, {day: shortArray[0].dayName}, { [name]: parseInt(values.reduce( (a,b) => a+b ) / values.length) }),{}
                    );
    ...
    const avgResult= func(propsArray.filter(Boolean), 8)   
